#!/usr/bin/env bash
#
# Restore a PostgreSQL logical backup in custom format (from scripts/pg_backup.sh).
...
#
# Usage:
#   PGDATABASE=rhododendra PGUSER=rhododendra ./scripts/pg_restore.sh /path/to/backup.dump
#
set -euo pipefail
